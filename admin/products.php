...
if (isset($_GET['add']) || (isset($_GET['edit']))) {
    ...
    if (isset($_GET['edit'])) {
        ...
        if (isset($_GET['delete_image'])) {
            ...
            header('Locaetion:produnct.php?edit=' . $edit_id);
        }
    ...
    if ($_POST) {
        ...
        if (!empty($errors)) {
            ...
            if ($tmpLoc && $uploadPath) {
                move_uploaded_file($tmpLoc, $uploadPath);
            }
            if (isset($_GET['edit'])) {
                $insertSql = "UPDATE product SET title='$title', price='$price', list_price='$list_price', brand='$brand', categories='$category', sizes='$sizes', ";
                if ($dbpath) {
                    $insertSql .= "image='$dbpath', ";
                }
                $insertSql .="description='$description' WHERE id=$edit_id";
            } else {
        ...
        <div class="form-group col-md-3">
            ...
            <select class="form-control" id="brand" name="brand">
                ...
                <?php while ($b = mysqli_fetch_assoc($brandQuery)): ?>
                    <option
                        value="<?= $b['id']; ?>"<?= (($brand == $b['id']) ? 'selected' : ''); ?>><?= $b['brand']; ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group col-md-3">
            ...
            <select class="form-control" id="parent" name="parent">
                ...
                <?php while ($p = mysqli_fetch_assoc($parentQuery)): ?>
                    <option
                        value="<?= $p['id'] ?>" <?= (($parent == $p['id']) ? 'selected' : ''); ?>><?= $p['category']; ?></option>
                <?php endwhile; ?>
                <option></option>
            </select>
        </div>
            ...
            <div class="modal-content">
                    ...
                    <div class="container-fluid">
                        <?php for ($i = 1; $i <= 12; $i++): ?>
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="size<?= $i; ?>" id="qty<?= $i; ?>"
                                       value="<?= ((!empty($sArray[$i - 1])) ? $sArray[$i - 1] : ''); ?>"
                                       class="form-control">
                            </div>
                            <div class="form-group col-md-2">
                                <label for="qty<?= $i; ?>">Quantity:</label>
                                <input type="number" name="qty<?= $i; ?>" id="size<?= $i; ?>"
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary"
                            onclick="updateSizes();jQuery('#sizesModal').modal('toggle');return false;">Save changes
                    </button>
                </div>
            </div>
        </div>
